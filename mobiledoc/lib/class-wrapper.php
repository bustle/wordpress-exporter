...
class Wrapper {
	...
	public function __construct( $post ) {
		...
		if ( is_a( $post, 'WP_Post' ) ) {
			foreach ( $this->get_nodes( $post->post_content ) as $node ) {
	...
	public function handle_list( $element ) {
		...
		foreach ( $element->childNodes as $node ) {
			...
			if ( ! is_a( $node, 'DOMElement' ) || $node->tagName !== 'li' ) {
				continue;
			}
		...
		if ( is_a( $element, 'DOMElement' ) ) {
			...
			switch ( $element->tagName) {
				...
				case 'span' :
					...
					if ( $element->hasAttribute( 'class' ) && strpos( $element->getAttribute( 'class' ), 's1' ) !== false ) {
	...
	public function get_card( $element ) {
		...
		if ( is_a( $element, 'DOMElement' ) ) {
			...
			// Filter components i.e. if a user has a custom
			// card they'd like to add
			$components = apply_filters( 'bustle_mobiledoc_components', $this->components );
